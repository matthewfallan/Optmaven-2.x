...
def load_input_files(experiment, directory):
    """ Load the Topology and Parameter input files in a CHARMM script. """
    lines = list()
    garbage = list()
    # Loop through topology and parameter data.
    input_files = [
        {"title": "topology" , "files": experiment.topology_files,  "cmd": "rtf" },
        {"title": "parameter", "files": experiment.parameter_files, "cmd": "para"}
    ]
    for file_type in input_files:
        lines.append("! Load the {} file(s)".format(file_type["title"]))
        for count, file_ in enumerate(file_type["files"]):
            ...
            symlink = os.path.join(directory, file_name)
            os.symlink(file_, symlink)
            garbage.append(symlink)
            lines.append("open read unit 10 form name {} card".format(file_name))
            # If there is more than one file, indicate that the subsequent files are being appended.
            append = " append" * int(count > 0)
            lines.append("read {} card unit 10{}".format(file_type["cmd"], append))
            lines.append("close unit 10")
    text = "\n".join(lines)
    return text, garbage
        

def load_molecules(experiment, directory):
    """ Create text to load Molecules in a CHARMM script. """
    lines = list()
    garbage = list()
    parser = PDBParser()
    molecule_files = experiment.get_molecule_files()
    # Loop through the Molecules
    for file_name, file_ in molecule_files.items():
        structure = parser.get_structure(file_name, file_)
